...
`features/review/api/reviewApi.ts` is the integration seam for the accepted backend transport. It sends the bounded source, language, mode, learner level, and optional title/context to `POST /api/v1/reviews` with credentials and an idempotency key; the API fingerprints and persists those admission fields, and the Luna prompt labels title/context/level as untrusted metadata. It sends an empty object to `POST /api/v1/reviews/:id/process`, reads `GET /api/v1/reviews/:id/result`, validates success envelopes and response shapes, and maps failures to a safe client error. The route still injects the deterministic demo factory when no authenticated API session is available, so the demo transport remains local and compatible.
