...
const UsageSourceNote: FC<UsageSourceNoteProps> = ({ source }): ReactElement => (
  <aside className="usage-source-note" data-transport-mode={source}>
    ...
    <p>
      {source === "demo"
        ? "Deterministic fixture values only. Cost values use a labeled demo pricing version; no authenticated session or live metrics are connected."
        : "Authenticated read-only usage requests use the accepted API contract. Cost is shown only from persisted server estimates; browser provider controls are not present."}
    </p>
  </aside>
);
